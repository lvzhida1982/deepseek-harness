...
function fakeAgent(ctx: Context, cwd: string): Agent {
  const id = SessionId('windows-world')
  const agent = {
    id,
    ctx: ctx.plugin(() => {}).ctx,
    session: {
...
describe('bash execution-world workdir', () => {
  it('resolves a relative model workdir using the execution-world path grammar', async () => {
    const { ctx, shell } = await harness()
    const agent = fakeAgent(ctx, String.raw`D:\Project`)

    const result = await ctx.tools.execute({
      signal: new AbortController().signal,
      callId: CallId('windows-relative-workdir'),
      name: 'bash',
      arguments: {
        command: 'echo ok',
